...
if [[ -z "$xui_version" ]]; then
    ...
    echo ""
    echo -e "${green}-------->>>>片刻之后脚本将会自动引导安装 Next-Panel${plain}"
else
    # 检查版本号中是否包含冒号
    if [[ "$xui_version" == *:* ]]; then
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}获取 Next-Panel 版本失败，可能是 Github API 限制，请稍后再试${plain}"
            exit 1
        fi
        echo ""
        echo -e "-----------------------------------------------------"
        echo -e "${green}--------->>获取 Next-Panel 最新版本：${yellow}${last_version}${plain}${green}，开始安装...${plain}"
        echo -e "-----------------------------------------------------"
        echo ""
        sleep 2
        ...
        echo ""
        sleep 2
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/sushen339/Next-Panel/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
            echo -e "${red}下载 Next-Panel 失败, 请检查服务器是否可以连接至 GitHub？ ${plain}"
            exit 1
        fi
    else
        last_version=$1
        url="https://github.com/sushen339/Next-Panel/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz"
        echo ""
        echo -e "--------------------------------------------"
        echo -e "${green}---------------->>>>开始安装 Next-Panel $1${plain}"
        echo -e "--------------------------------------------"
        echo ""
        sleep 2
        echo -e "${green}---------------->>>>>>>>>安装进度50%${plain}"
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}下载 Next-Panel $1 失败, 请检查此版本是否存在 ${plain}"
            exit 1
        fi
    fi
    wget -O /usr/bin/x-ui-temp https://raw.githubusercontent.com/sushen339/Next-Panel/main/x-ui.sh
...
ssh_forwarding
    ...
    # 安装服务文件 - Alpine 使用 OpenRC，其他系统使用 systemd
    if [[ "${release}" == "alpine" ]]; then
        cp -f x-ui.initd /etc/init.d/x-ui
    ...
    # 停止 warp-go 服务
    if [[ "${release}" == "alpine" ]]; then
        rc-service warp-go stop >/dev/null 2>&1 || true
    else
    ...
    # 启动 warp-go 服务
    if [[ "${release}" == "alpine" ]]; then
        rc-service warp-go start >/dev/null 2>&1 || true
    else
    ...
    echo ""
    echo -e "------->>>>${green}Next-Panel ${last_version}${plain}<<<<安装成功，正在启动..."
    sleep 1
    echo ""
    echo -e "         ---------------------"
    ...
    echo -e "x-ui stop         - 关闭 Next-Panel 面板"
    ...
    echo -e ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
    echo ""
    echo -e "${yellow}----->>>Next-Panel和Xray启动成功<<<-----${plain}"
}
