...
if [[ -f /etc/os-release ]]; then
    ...
    echo -e "${red}检查服务器操作系统失败，请检查系统版本!${plain}" >&2
    exit 1
fi
...
last_version=$(curl -Ls "https://api.github.com/repos/sushen339/Next-Panel/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
echo -e "${green}当前代理面板的版本为: ${red}Next-Panel v${xui_version}${plain}"
echo ""
echo -e "${yellow}Next-Panel 最新版为---------->>> ${last_version}${plain}"

os_version=$(grep -i version_id /etc/os-release | cut -d \" -f2 | cut -d . -f1)
os_codename=$(grep -i version_codename /etc/os-release | cut -d = -f2)
...
if [[ "${release}" == "centos" ]]; then
    ...
    # 支持 Debian sid/forky（不稳定版），允许其通过版本检查
    if [[ "${os_codename}" == "sid" || "${os_codename}" == "forky" ]]; then
        echo "您的操作系统是 Debian sid (滚动更新版)"
    elif [[ -n "${os_version}" && ${os_version} -lt 11 ]]; then
...
service_reload() {
    if [[ "${release}" == "alpine" ]]; then
        rc-service x-ui reload 2>/dev/null || rc-service x-ui restart
    else
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/sushen339/Next-Panel/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/sushen339/Next-Panel/main/x-ui.sh
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/sushen339/Next-Panel/main/install.sh"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/sushen339/Next-Panel/main/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    echo -e "${green} 请使用新的登录用户名和密码访问 Next-Panel 面板。也请记住它们！${plain}"
    confirm_restart
}
...
check_config() {
    ...
    if [[ -z "$existing_cert" && -z "$existing_key" ]]; then
        ...
        if [[ -z $v4 ]]; then
            ...
            echo ""
            echo -e "${green}4、请在浏览器地址栏复制${plain} ${blue}[::1]:15208${existing_webBasePath}${plain} ${green}进入 Next-Panel 登录界面"
            echo ""
            echo -e "${red}注意：若不使用〔ssh转发〕请为Next-Panel面板配置安装证书再行登录管理后台${plain}"
        elif [[ -n $v4 && -n $v6 ]]; then
            ...
            echo -e "${green}3、请在终端中成功输入服务器的〔root密码〕，注意区分大小写，用以上命令进行转发${plain}"
            echo ""
            echo -e "${green}4、请在浏览器地址栏复制${plain} ${blue}127.0.0.1:15208${existing_webBasePath}${plain} ${yellow}或者${plain} ${blue}[::1]:15208${existing_webBasePath}${plain} ${green}进入 Next-Panel 登录界面"
            echo ""
            echo -e "${red}注意：若不使用〔ssh转发〕请为Next-Panel面板配置安装证书再行登录管理后台${plain}"
        else
            ...
            echo -e "${green}3、请在终端中成功输入服务器的〔root密码〕，注意区分大小写，用以上命令进行转发${plain}"
            echo ""
            echo -e "${green}4、请在浏览器地址栏复制${plain} ${blue}127.0.0.1:15208${existing_webBasePath}${plain} ${green}进入 Next-Panel 登录界面"
            echo ""
            echo -e "${red}注意：若不使用〔ssh转发〕请为Next-Panel面板配置安装证书再行登录管理后台${plain}"
            echo ""
        fi
...
stop_panel_only() {
    ...
    if [[ $? == 1 ]]; then
        ...
        # 创建标记文件，告诉面板只停止Web服务，保留Xray运行
        touch /usr/local/x-ui/bin/stop_panel_only.flag
        
        # 记录当前Xray进程PID（如果存在）
        xray_pid=$(pgrep -f "xray-linux.*config.json" | head -1)
        ...
        # 清理标记文件
        rm -f /usr/local/x-ui/bin/stop_panel_only.flag
        
        # 检查Xray是否还在运行（使用之前记录的PID或搜索进程）
        if [[ -n "$xray_pid" ]] && ps -p "$xray_pid" > /dev/null 2>&1; then
            LOGI "Next-Panel 面板已关闭，Xray 核心继续运行 (PID: $xray_pid)"
            LOGI "注意: 此时无法通过面板管理 Xray，但代理服务正常"
            LOGI "使用 'x-ui start' 或菜单选项11重新启动面板以恢复管理"
        elif pgrep -f "xray-linux.*config.json" > /dev/null 2>&1; then
            new_pid=$(pgrep -f "xray-linux.*config.json" | head -1)
            LOGI "Next-Panel 面板已关闭，Xray 核心继续运行 (PID: $new_pid)"
            LOGI "注意: 此时无法通过面板管理 Xray，但代理服务正常"
            LOGI "使用 'x-ui start' 或菜单选项11重新启动面板以恢复管理"
        else
            LOGE "警告: Xray 核心也已停止"
            LOGE "原因: Xray可能未启动或配置有误"
            LOGE "建议: 使用选项11启动面板并检查Xray配置"
        fi
    fi
...
show_log() {
    if [[ "${release}" == "alpine" ]]; then
        tail -f /var/log/x-ui.log 2>/dev/null || echo -e "${red}日志文件不存在${plain}"
    else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/sushen339/Next-Panel/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_enabled() {
    if [[ "${release}" == "alpine" ]]; then
        rc-update show default | grep -q x-ui
        return $?
    else
...
update_geo() {
    ...
    service_start
    echo -e "${green}Geosite.dat + Geoip.dat 在 bin 文件夹: '${binfolder}' 中已经更新成功 !${plain}"
    before_show_menu
}
...
create_iplimit_jails() {
    ...
    if [[  "${release}" == "debian" && -n "${os_version}" && ${os_version} -ge 12 ]]; then
...
show_usage() {
    echo -e "         ---------------------"
    echo -e "         |${green}Next-Panel 控制菜单用法 ${plain}|${plain}"
    echo -e "         | ${yellow}基于Xray Core构建 ${plain}|${plain}"  
    echo -e "--------------------------------------------"
    echo -e "x-ui              - 进入管理脚本"
    echo -e "x-ui start        - 启动 Next-Panel 面板"
    echo -e "x-ui stop         - 关闭 Next-Panel 面板和核心"
    echo -e "x-ui stop-panel   - 仅关闭面板 ${yellow}(保留核心运行)${plain}"
    echo -e "x-ui restart      - 重启 Next-Panel 面板"
    echo -e "x-ui status       - 查看 Next-Panel 状态"
    echo -e "x-ui settings     - 查看当前设置信息"
    echo -e "x-ui enable       - 启用 Next-Panel 开机启动"
    echo -e "x-ui disable      - 禁用 Next-Panel 开机启动"
    echo -e "x-ui log          - 查看 Next-Panel 运行日志"
    echo -e "x-ui banlog       - 检查 Fail2ban 禁止日志"
    echo -e "x-ui update       - 更新 Next-Panel 面板"
    echo -e "x-ui custom       - 自定义 Next-Panel 版本"
    echo -e "x-ui install      - 安装 Next-Panel 面板"
    echo -e "x-ui uninstall    - 卸载 Next-Panel 面板"
    echo -e "--------------------------------------------"
}
...
——————————————————————
  ...
  ${green}12.${plain} 关闭面板
  ${green}13.${plain} 重启面板
  ${green}14.${plain} 仅关闭面板 ${yellow}(保留核心运行)${plain}
  ${green}15.${plain} 检查面板状态
  ${green}16.${plain} 检查面板日志
——————————————————————
...
if [[ $# > 0 ]]; then
    ...
    "stop")
        ...
        ;;
    "stop-panel")
        check_install 0 && stop_panel_only 0
        ;;
    "restart")
