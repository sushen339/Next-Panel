...
func runWebServer() {
	...
	// 移除独立的订阅服务器，现在已集成到 web 服务器中
	logger.Info("Subscription service is now integrated into the main web server")
...
//    ipv6 := strings.TrimSpace(string(outputIPv6))
 ...
 // 		fmt.Println("")
 // 	}
	fmt.Println(Green + ">>>>>>>>注：若您安装了证书，请使用您的域名用https方式登录" + Reset)
	fmt.Println("")
	fmt.Println("--------------------------------------------------")
	fmt.Println("")
//	fmt.Println("↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑")
