...
func (s *Server) initRouter() (*gin.Engine, error) {
	...
	engine.Use(sessions.Sessions("next-panel", store))
	...
	// 初始化订阅服务
	err = s.initSubController(engine)
	if err != nil {
		logger.Warning("Failed to initialize subscription controller:", err)
	}

	return engine, nil
}

func (s *Server) initSubController(engine *gin.Engine) error {
	// 检查订阅服务是否启用
	...
	// 初始化订阅控制器
	s.sub = sub.NewSUBController(
		g, LinksPath, JsonPath, Encrypt, ShowInfo, RemarkModel, SubUpdates,
		SubJsonFragment, SubJsonNoises, SubJsonMux, SubJsonRules, SubTitle)

	logger.Info("Subscription service integrated into web server")
	return nil
}

func (s *Server) startTask() {
	// 检查是否有外部 xray 进程在运行
	hasExternalXray := s.xrayService.IsXrayRunning() && !s.xrayService.HasManagedXray()
	
	if hasExternalXray {
		logger.Info("检测到外部 xray 进程，尝试连接...")
		// 尝试通过 API 连接现有的 xray 进程
		// 如果 API 可用，说明是"仅关闭面板"后的正常进程，可以接管
		if s.xrayService.IsXrayApiAvailable() {
			logger.Info("成功连接到现有 xray 进程，接管管理")
			s.xrayService.TakeoverExternalXray()
		} else {
			logger.Warning("检测到孤儿 xray 进程且 API 不可用，强制重启")
			s.xrayService.RestartXray(true)
		}
	} else {
		// 正常启动 xray
...
func (s *Server) Stop() error {
	...
	// 检查是否存在"仅停止面板"标记文件
	flagFile := config.GetBinFolderPath() + "/stop_panel_only.flag"
	stopPanelOnly := false
	if _, err := os.Stat(flagFile); err == nil {
		stopPanelOnly = true
		logger.Info("检测到仅停止面板标记，Xray核心将继续运行")
	}
	
	// 只有在不是"仅停止面板"模式时才停止Xray
	if !stopPanelOnly {
		s.xrayService.StopXray()
	} else {
		logger.Info("Xray核心继续运行，面板Web服务关闭")
	}
