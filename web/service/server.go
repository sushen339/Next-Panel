...
func (s *ServerService) ImportDB(file multipart.File) error {
	...
	// Step 1: Validate SQLite format
	logger.Debug("Step 1: Validating SQLite file format...")
	isValidDb, err := database.IsSQLiteDB(file)
	if err != nil {
		logger.Error("Failed to read database file header:", err)
		return common.NewErrorf("Error checking db file format: %v", err)
	}
	if !isValidDb {
		logger.Error("Invalid database file: not a valid SQLite format")
		return common.NewError("Invalid db file format: file is not a valid SQLite database")
	}
	logger.Info("Database file format validation passed")

	// Reset the file reader to the beginning
	_, err = file.Seek(0, 0)
	if err != nil {
		logger.Error("Failed to reset file reader:", err)
		return common.NewErrorf("Error resetting file reader: %v", err)
	}

	// Save the file as a temporary file
	tempPath := fmt.Sprintf("%s.temp", config.GetDBPath())
	logger.Debugf("Step 2: Saving uploaded file to temporary location: %s", tempPath)

	// Remove the existing temporary file (if any)
	if _, err := os.Stat(tempPath); err == nil {
		logger.Debug("Removing existing temporary file...")
		if errRemove := os.Remove(tempPath); errRemove != nil {
			logger.Error("Failed to remove existing temporary file:", errRemove)
	...
	logger.Infof("Successfully saved %d bytes to temporary file", bytesWritten)
	...
	tempFile = nil // Prevent double close in defer

	// Step 3: Validate database integrity
	logger.Debug("Step 3: Validating database integrity...")
	if err = database.ValidateDB(tempPath); err != nil {
		logger.Error("Database integrity validation failed:", err)
		return common.NewErrorf("Error validating db integrity: %v", err)
	}
	logger.Info("Database integrity validation passed")

	// Step 4: Test database initialization
	logger.Debug("Step 4: Testing database initialization...")
	if err = database.InitDB(tempPath); err != nil {
		logger.Error("Database initialization test failed:", err)
		return common.NewErrorf("Error checking db: %v", err)
	}
	logger.Info("Database initialization test passed")

	// Step 5: Stop Xray service
	logger.Info("Step 5: Stopping Xray service...")
	if err = s.StopXrayService(); err != nil {
		logger.Warning("Failed to stop Xray service (continuing anyway):", err)
	}

	// Step 6: Backup current database
	fallbackPath := fmt.Sprintf("%s.backup", config.GetDBPath())
	logger.Infof("Step 6: Backing up current database to: %s", fallbackPath)

	// Remove the existing fallback file (if any)
	if _, err := os.Stat(fallbackPath); err == nil {
		logger.Debug("Removing existing backup file...")
		if errRemove := os.Remove(fallbackPath); errRemove != nil {
			logger.Error("Failed to remove existing backup file:", errRemove)
	...
	if err = os.Rename(config.GetDBPath(), fallbackPath); err != nil {
		logger.Error("Failed to backup current database:", err)
		return common.NewErrorf("Error backing up current db file: %v", err)
	}
	logger.Info("Current database backed up successfully")
	...
	// Step 7: Replace with new database
	logger.Info("Step 7: Replacing database with imported file...")
	if err = os.Rename(tempPath, config.GetDBPath()); err != nil {
		logger.Error("Failed to move new database file, restoring backup:", err)
		// Restore from fallback
		if errRename := os.Rename(fallbackPath, config.GetDBPath()); errRename != nil {
			logger.Error("CRITICAL: Failed to restore backup database:", errRename)
	...
	// Step 8: Migrate new database
	logger.Info("Step 8: Migrating imported database...")
	if err = database.InitDB(config.GetDBPath()); err != nil {
		logger.Error("Database migration failed, restoring backup:", err)
		if errRename := os.Rename(fallbackPath, config.GetDBPath()); errRename != nil {
			logger.Error("CRITICAL: Failed to restore backup database:", errRename)
	...
	// Step 9: Restart Xray service
	logger.Info("Step 9: Restarting Xray service...")
	if err = s.RestartXrayService(); err != nil {
		logger.Error("Failed to restart Xray service:", err)
