...
func (s *XrayService) IsXrayRunning() bool {
	...
	// 如果进程对象为空或显示未运行,则检查系统中是否有 xray 进程
	// 这种情况发生在"仅关闭面板"后重启面板时
	return xray.IsXrayProcessRunning()
}

// HasManagedXray 检查是否有由面板管理的 xray 进程
func (s *XrayService) HasManagedXray() bool {
	return p != nil && p.IsRunning()
}

// TakeoverExternalXray 接管外部 xray 进程（仅更新API连接，不重新创建进程对象）
// 因为我们无法获取外部进程的Process对象，所以只能通过API与之通信
// 当需要重启时，会先kill掉外部进程再启动新的
func (s *XrayService) TakeoverExternalXray() {
	// 尝试重新初始化 API 连接
...
// IsXrayApiAvailable 检查 Xray API 是否可用
// 用于判断是否可以与现有的 xray 进程通信
func (s *XrayService) IsXrayApiAvailable() bool {
	// 首先尝试从进程对象获取端口
	apiPort := s.GetApiPort()
	
	// 如果进程对象不存在（仅关闭面板后重启的情况），从配置文件读取
	if apiPort <= 0 {
		apiPort = xray.GetApiPortFromConfig()
	}
	...
	// 尝试初始化 API 连接
	tempAPI := &xray.XrayAPI{}
	err := tempAPI.Init(apiPort)
	if err != nil {
		return false
	}
	defer tempAPI.Close()
	
	// 尝试获取流量统计，如果成功说明 API 可用
	_, _, err = tempAPI.GetTraffic(false)
	return err == nil
}
...
func (s *XrayService) StopXray() error {
	...
	// 如果没有进程对象（接管的外部进程），使用 pkill 强制终止
	logger.Info("Xray 进程由外部启动，使用 pkill 终止...")
	return xray.KillXrayProcess()
}
