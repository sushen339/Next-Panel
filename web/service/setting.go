...
func (s *SettingService) GetSecret() ([]byte, error) {
	...
	// If secret doesn't exist or is empty, generate a new secure one
	if err != nil || secret == "" {
...
func (s *SettingService) GetSubTotalId() (string, error) {
	return s.getString("subTotalId")
}

func (s *SettingService) SetSubTotalId(subTotalId string) error {
	return s.saveSetting("subTotalId", subTotalId)
}
...
func (s *SettingService) GetDefaultSettings(host string) (any, error) {
	...
	settings := map[string]settingFunc{
		...
		"subTotalId":    func() (any, error) { return s.GetSubTotalId() },
	...
	if result["subEnable"].(bool) && (result["subURI"].(string) == "" || result["subJsonURI"].(string) == "") {
		...
		webPort, _ := s.GetPort()  // 使用面板端口而不是订阅端口
		subPath, _ := s.GetSubPath()
		subJsonPath, _ := s.GetSubJsonPath()
		subDomain, _ := s.GetSubDomain()
		webKeyFile, _ := s.GetKeyFile()      // 使用面板证书
		webCertFile, _ := s.GetCertFile()    // 使用面板证书
		subTLS := false
		if webKeyFile != "" && webCertFile != "" {
			subTLS = true
		}
		if subDomain == "" {
		...
		if (webPort == 443 && subTLS) || (webPort == 80 && !subTLS) {
			subURI += subDomain
		} else {
			subURI += fmt.Sprintf("%s:%d", subDomain, webPort)
		}
