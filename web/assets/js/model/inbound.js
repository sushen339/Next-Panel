...
class RealityStreamSettings extends XrayCommonClass {
    constructor(
        ...
        mldsa65Seed = '',
        limitFallbackUpload = null,
        limitFallbackDownload = null,
    ...
    static fromJson(json = {}) {
        ...
        // 将limitFallback对象转换为UI使用的字符串格式
        let limitFallbackUpload = '';
        if (json.limitFallbackUpload && typeof json.limitFallbackUpload === 'object') {
            limitFallbackUpload = `${json.limitFallbackUpload.afterBytes}-${json.limitFallbackUpload.bytesPerSec}-${json.limitFallbackUpload.burstBytesPerSec}`;
        } else if (typeof json.limitFallbackUpload === 'string') {
            limitFallbackUpload = json.limitFallbackUpload;
        }
        
        let limitFallbackDownload = '';
        if (json.limitFallbackDownload && typeof json.limitFallbackDownload === 'object') {
            limitFallbackDownload = `${json.limitFallbackDownload.afterBytes}-${json.limitFallbackDownload.bytesPerSec}-${json.limitFallbackDownload.burstBytesPerSec}`;
        } else if (typeof json.limitFallbackDownload === 'string') {
            limitFallbackDownload = json.limitFallbackDownload;
        }
    ...
    toJson() {
        ...
        // 处理limitFallback配置，输入格式为字符串"afterBytes-bytesPerSec-burstBytesPerSec"
        if (this.limitFallbackUpload && typeof this.limitFallbackUpload === 'string' && this.limitFallbackUpload.trim()) {
            const parts = this.limitFallbackUpload.split('-').map(v => parseInt(v.trim()));
            if (parts.length === 3 && parts.every(v => !isNaN(v))) {
                obj.limitFallbackUpload = {
                    afterBytes: parts[0],
                    bytesPerSec: parts[1],
                    burstBytesPerSec: parts[2]
                };
            }
        } else if (this.limitFallbackUpload && typeof this.limitFallbackUpload === 'object') {
            obj.limitFallbackUpload = this.limitFallbackUpload;
        }
        
        if (this.limitFallbackDownload && typeof this.limitFallbackDownload === 'string' && this.limitFallbackDownload.trim()) {
            const parts = this.limitFallbackDownload.split('-').map(v => parseInt(v.trim()));
            if (parts.length === 3 && parts.every(v => !isNaN(v))) {
                obj.limitFallbackDownload = {
                    afterBytes: parts[0],
                    bytesPerSec: parts[1],
                    burstBytesPerSec: parts[2]
                };
            }
        } else if (this.limitFallbackDownload && typeof this.limitFallbackDownload === 'object') {
            obj.limitFallbackDownload = this.limitFallbackDownload;
        }
        return obj;
    }
}
...
class SockoptStreamSettings extends XrayCommonClass {
    ...
    toJson() {
        ...
        // 只在有值时添加happyEyeballs
        if (this.happyEyeballs && this.happyEyeballs.tryDelayMs > 0) {
            obj.happyEyeballs = this.happyEyeballs;
        }
        return obj;
    }
}
...
class Sniffing extends XrayCommonClass {
    ...
    static fromJson(json = {}) {
        ...
        // 如果从JSON加载，domainsExcluded是数组，转换为字符串供UI使用
        let domainsExcluded = '';
        if (json.domainsExcluded && Array.isArray(json.domainsExcluded)) {
            domainsExcluded = json.domainsExcluded.join('\n');
        }
        return new Sniffing(
            !!json.enabled,
            destOverride,
    ...
    toJson() {
        ...
        if (this.metadataOnly) obj.metadataOnly = this.metadataOnly;
        if (this.routeOnly) obj.routeOnly = this.routeOnly;
        // 将换行分隔的字符串转换为数组
        if (this.domainsExcluded && this.domainsExcluded.trim().length > 0) {
            obj.domainsExcluded = this.domainsExcluded.split('\n')
                .map(d => d.trim())
                .filter(d => d.length > 0);
        }
        return obj;
