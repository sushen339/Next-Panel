...
class Outbound extends CommonClass {
    ...
    static fromLink(link) {
        ...
        switch (data[0].toLowerCase()) {
            ...
            case 'ss':
                // SS链接有多种格式，需要特殊处理
                return this.fromSSLink(link);
            default:
                return null;
        }
    }

    static fromSSLink(link) {
        try {
            // SS链接格式：
            // 1. ss://base64(method:password@host:port)#remark (传统SIP002)
            // 2. ss://base64(method:password)@host:port#remark (SIP002变体，包括SS2022)
            // 3. ss://method:password@host:port#remark (SS2022明文格式)
            
            const url = new URL(link);
            let userInfo = url.username;
            let address = url.hostname;
            let port = parseInt(url.port);
            let remark = decodeURIComponent(url.hash.substring(1)) || `ss-${port}`;
            
            let method, password;
            
            if (url.password) {
                // 格式2或3: method在username，password在password字段
                method = decodeURIComponent(userInfo);
                password = decodeURIComponent(url.password);
            } else {
                // 格式1或2: userInfo可能是base64编码的
                try {
                    // 修复base64 padding（如果缺失）
                    let paddedUserInfo = userInfo;
                    const padLength = (4 - userInfo.length % 4) % 4;
                    if (padLength > 0) {
                        paddedUserInfo = userInfo + '='.repeat(padLength);
                    }
                    
                    let decoded = atob(paddedUserInfo);
                    
                    // 解析 method:password@host:port 或 method:password 或 method:serverPassword:userPassword
                    if (decoded.includes('@')) {
                        // 格式1: 完整编码 method:password@host:port
                        const parts = decoded.split('@');
                        const authParts = parts[0].split(':');
                        method = authParts[0];
                        password = authParts.slice(1).join(':');
                        address = parts[1].split(':')[0];
                        port = parseInt(parts[1].split(':')[1]);
                    } else {
                        // 格式2: 只编码认证部分 method:password 或 method:serverPass:userPass
                        const authParts = decoded.split(':');
                        method = authParts[0];
                        // SS2022可能有多个冒号分隔的密码部分，全部合并
                        password = authParts.slice(1).join(':');
                    }
                } catch (e) {
                    // base64解码失败，可能是明文格式（SS2022）
                    console.warn('Base64 decode failed, trying plain text:', e);
                    const authParts = userInfo.split(':');
                    method = authParts[0];
                    password = authParts.slice(1).join(':');
                }
            }
            
            let stream = new StreamSettings('tcp', 'none');
            let settings = new Outbound.ShadowsocksSettings(address, port, password, method, false);
            
            return new Outbound(remark, Protocols.Shadowsocks, settings, stream);
        } catch (e) {
    ...
    static fromParamLink(link) {
        ...
        if (protocol == 'ss') {
            ...
            // SS链接解析：支持SS2022和传统格式
            // 格式1: ss://base64(method:password)@host:port
            // 格式2: ss://method:password@host:port (SS2022新格式)
            try {
                // 尝试base64解码（传统格式）
                let decoded = atob(userData);
                userData = decoded.split(':');
            } catch (e) {
                // 如果解码失败，说明可能是明文格式（SS2022）
                // 直接按冒号分割
                userData = userData.split(':');
            }
            
            // userData现在是[method, password]或[method, ...password_parts]
            let method = userData.shift(); // 第一个是method
            let password = userData.join(':'); // 剩余部分用冒号连接（支持密码中包含冒号）
            settings = new Outbound.ShadowsocksSettings(address, port, password, method, true);
        } else {
            switch (protocol) {
                case Protocols.VLESS:
...
Outbound.VLESSSettings = class extends CommonClass {
    constructor(address = '', port = 443, id = '', flow = '', encryption = 'none', encryptionType = 'none') {
        super();
        this.address = address;
        this.port = port;
        this.id = id;
        ...
        // 根据encryption值推断encryptionType（如果未明确指定）
        if (encryptionType === 'none' && encryption && encryption !== 'none') {
            // 如果encryption有值但encryptionType是none，需要推断
            if (encryption.includes('mlkem768')) {
                this.encryptionType = 'mlkem768';
            } else if (encryption.length > 20) {
                this.encryptionType = 'custom';
            } else {
                this.encryptionType = 'x25519';
            }
        } else {
    ...
    static fromJson(json = {}) {
        ...
        const encryption = json.vnext[0].users[0].encryption || 'none';
    ...
    toJson() {
        // 根据encryptionType决定实际的encryption值
        let finalEncryption = 'none';
        
        if (this.encryptionType === 'none') {
            finalEncryption = 'none';
        } else if (this.encryptionType === 'custom') {
            // 自定义类型使用用户输入的值
            finalEncryption = this.encryption || 'none';
        } else if (this.encryptionType === 'x25519' || this.encryptionType === 'mlkem768') {
            // x25519和mlkem768类型使用生成的值（如果有的话）
            finalEncryption = this.encryption || 'none';
        }
...
Outbound.ShadowsocksSettings = class extends CommonClass {
    constructor(address = '', port = 8388, password = '', method = 'aes-256-gcm', uot = false, UoTVersion = 2) {
        super();
        this.address = address;
        this.port = port;
        this.password = password;
